...
TEST_SPLIT = 8  # means 0.1 of data is for test
...
df = pd.DataFrame(
    source,
    columns=["sid", "aid", "wid", "mid"] + all_feat_x + strings_x + strings_y +
    ["embeddings"] + feat_y)
...
dumm = pd.get_dummies(df, columns=all_feat_x + feat_y)
...
embeddingInputSets = {i: getSet(df[i]) for i in all_feat_x}

# embeddingInputSets

all_feat_x = list(
    set(all_feat_x) -
    set([x for x in all_feat_x if len(embeddingInputSets[x]) <= 1]))
...
for L in range(0, len(tools)+1):
    for subset in itertools.combinations(tools, L):
        feat_x = [s for s in all_feat_x if s.startswith(subset)]

        np.random.seed(0)
        subset_name = ",".join(subset)
        print("subset=", subset_name, flush=True)
        # print("feat_x=", feat_x)
        thedate = (str(subset_name) +
        ...
        data = {
            ...
            getData(train, values_train, cats_feats=feat_x, strs_feats=strings_x,
                    embeddings=WORD_EMB),
            'output':
            getData(train, values_train, cats_feats=feat_y,
        ...
        print('\nTraining Data:')
        print("\n".join(["X: " + i + str(data["input"][i].shape)
                         for i in feat_x]))
        print("\n".join(
        ...
        if len(inputs) >= 1:
            main_input = layers.Concatenate()(
                [getEmbedding(input, feat_x) for input in inputs])
        ...
        for ite in range(100):
            print("\nPredicted String Q",
                  ctable.decode(mydata[0][strings_x[0]]
                                [ite], calc_argmax=True),
